<x-home>
    <div class="container mx-auto p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden transition-transform hover:scale-[1.02] flex flex-col h-full text-sm">
                <!-- Foto -->
                ...
                <div class="p-5 flex flex-col flex-1 justify-between">
                    <div class="space-y-2">
                        ...
                        <div class="text-gray-500 break-all">UUID - {{ $dokumentasi->laporan_uuid }}</div>
                        <div class="text-gray-500">📍 Lokasi: <span class="text-gray-700">{{ $dokumentasi->laporan->lokasi }}</span></div>
                        <div class="text-gray-500">👨‍🔧 Teknisi: <span class="text-gray-700">{{ $dokumentasi->teknisi->nama }}</span></div>
                    ...
                    <div class="mt-4 flex justify-between items-center text-sm">
                        <a href="{{ $dokumentasi->foto_url }}" target="_blank"
                            class="inline-flex items-center gap-1 bg-indigo-600 text-white px-2.5 py-1 rounded hover:bg-indigo-700 transition text-xs shadow-sm">
                            🔍 Lihat Foto
                        </a>

                        <button onclick="showModal({{ $loop->index }})"
                            class="inline-flex items-center gap-1 bg-gray-100 text-gray-800 px-2.5 py-1 rounded hover:bg-gray-200 transition text-xs shadow-sm">
                            📄 Lihat Detail
                        </button>
                    </div>
                </div>
            ...
            <div id="modal-{{ $loop->index }}" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden items-center justify-center">
                <div class="bg-white rounded-xl w-full max-w-md p-6 shadow-xl relative text-sm">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="pt-2">
                            <img src="{{ $dokumentasi->foto_url }}" alt="Foto Dokumentasi"
                                 class="rounded-lg border shadow w-full max-h-64 object-contain">
                        </div>
                    </div>
                </div>
            </div>
