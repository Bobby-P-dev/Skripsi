<x-home>
                ...
                <div class="p-5 flex-1 flex flex-col justify-between">
                    ...
                    <div class="mt-3 flex justify-between items-center">
                        <button
                            ...
                            data-userfoto="{{ isset($laporan->pengguna) && $laporan->pelanggan->foto_profil ? asset('storage/' . $laporan->user->profile_photo_path) : 'https://ui-avatars.com/api/?name=' . urlencode($laporan->pelanggan->foto_profil ?? 'User') . '&background=4f46e5&color=fff&size=32' }}"
                            data-username="{{ $laporan->pelanggan->nama ?? 'Anonim' }}"
                        ...
                        <button
                            ...
                            data-deskripsi="{{ $laporan->deskripsi }}"
                            data-lokasi="{{ $laporan->lokasi }}"
                            data-tingkat_urgensi="{{ $laporan->tingkat_urgensi }}"
                            data-foto_url="{{ $laporan->foto_url ?? '' }}" {{-- Tambahkan ?? '' untuk fallback jika null --}}
                            data-action_url="{{ route('laporan.update', $laporan->laporan_uuid) }}"
                            title="Edit Laporan">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const openBuatLaporanBtn = document.getElementById('openModalBtn');
            const closeBuatLaporanBtn = document.getElementById('closeModalBtn');

            if (openBuatLaporanBtn) {
                openBuatLaporanBtn.addEventListener('click', () => {
            ...
            const laporanUuidInput = document.getElementById('laporan_uuid_edit');
            const judulInput = document.getElementById('judul_edit');
            const deskripsiInput = document.getElementById('deskripsi_edit');
            const lokasiInput = document.getElementById('lokasi_edit');
            const urgensiSelect = document.getElementById('urgensi_edit');
            const currentFotoPreviewImg = document.getElementById('current_foto_preview');
            const fotoEditInput = document.getElementById('foto_edit');

            if (editModalElement && editLaporanForm) {
                ...
                const openEditModal = (button) => {
                    ...
                    const tingkatUrgensi = button.dataset.tingkat_urgensi;
                    const fotoUrl = button.dataset.foto_url;
                    const actionUrl = button.dataset.action_url;

                    editLaporanForm.action = actionUrl;
                    if (laporanUuidInput) laporanUuidInput.value = uuid;
                    ...
                    if (urgensiSelect) urgensiSelect.value = tingkatUrgensi;

                    if (currentFotoPreviewImg) {
                    ...
                    if (fotoEditInput) {
                        fotoEditInput.value = null;
                    }

                    editModalElement.classList.add('flex');
                    editModalElement.classList.remove('hidden');
                };

                const closeEditModal = () => {
                    editModalElement.classList.add('hidden');
                    editModalElement.classList.remove('flex');
                };
